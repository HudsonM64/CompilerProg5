...
class IfThenElseExp extends Exp {
  ...
  Tree.Stm unCx(Label tt, Label ff) {
    ...
    if (bStm instanceof Tree.JUMP) {
      ...
      if (bJump.exp instanceof Tree.NAME) {
          Tree.NAME bName = (Tree.NAME)bJump.exp; //casting to name
          bStm = null;
          f = bName.label;
      }
  }
  ...
  Tree.Exp unEx() {
    ...
    if (bExp == null)
      ...
      return new Tree.ESEQ(new Tree.SEQ(new Tree.SEQ(cond.unCx(t, f), new Tree.SEQ(new Tree.SEQ(new Tree.LABEL(t), new Tree.SEQ(new Tree.MOVE(new Tree.TEMP(temp), aExp), new Tree.JUMP(join))), new Tree.SEQ(new Tree.LABEL(f), new Tree.SEQ(new Tree.MOVE(new Tree.TEMP(temp), bExp), new Tree.JUMP(join))))), new Tree.LABEL(join)), new Tree.TEMP(temp)); 
  }
